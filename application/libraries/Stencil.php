...
class Stencil {
	...
	protected $theme		= '';
	protected $no_visible_elements = false;
	...
	public function paint($page, $data = NULL)
	{
		...
		$this->data['no_visible_elements'] 		= $this->no_visible_elements;
		...
		foreach ($this->slice as $key => $value)
		{
			if (is_array($value))
			{
				foreach ($value as $k => $v)
				{
					...
					$this->data[$k] = $this->CI->load->view($this->theme.'/slices/'.$v, $this->data, TRUE)."\n";
				}
			}
			elseif (!is_numeric($key))
			{
				...
				$this->data[$key] = $this->CI->load->view($this->theme.'/slices/'.$value, $this->data, TRUE)."\n";
			}
			else
			{
				...
				$this->data[$value] = $this->CI->load->view($this->theme.'/slices/'.$value, $this->data, TRUE)."\n";
			}
		}
		$this->data['content'] = $this->CI->load->view($this->theme.'/pages/'.$page, $this->data, TRUE)."\n";
		$this->CI->load->view($this->theme.'/layouts/'.$this->layout, $this->data);
	}
	...
	public function no_visible_elements($no_visible_elements)
	{
		$this->no_visible_elements = $no_visible_elements;
	}
}
